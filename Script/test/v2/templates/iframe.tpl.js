      ...
      <div class="tpl-field">
        ...
        <div class="tpl-color-field">
          <div class="tpl-color-preview" style="background: ${value};">
            <input type="color" class="tpl-color-input" id="${inputId}-picker" name="color_${key}" data-color-field="${key}" value="${value}" aria-label="${label} - sélecteur">
          </div>
    ...
    return `
      <div class="tpl-form-section">
        ...
        <div class="tpl-form-grid single">
          <div class="tpl-field">
            <label class="tpl-label" for="tpl-iframe-title">Titre de la fenêtre</label>
            <input type="text" class="tpl-input" id="tpl-iframe-title" name="title" data-field="title" value="${helpers.escapeHtml(
              data.title || ""
            )}" placeholder="Titre" autocomplete="off">
          </div>
          <div class="tpl-field">
            <label class="tpl-label" for="tpl-iframe-url">URL du site à intégrer</label>
            <input type="url" class="tpl-input" id="tpl-iframe-url" name="url" data-field="url" value="${helpers.escapeHtml(
              data.url || ""
        ...
        <div class="tpl-form-grid">
          <div class="tpl-field">
            <label class="tpl-label" for="tpl-iframe-icon">Icône (emoji)</label>
            <input type="text" class="tpl-input" id="tpl-iframe-icon" name="icon" data-field="icon" value="${helpers.escapeHtml(
              data.icon || "🌐"
            )}" maxlength="2" style="width: 80px;" autocomplete="off">
          </div>
          <div class="tpl-field">
            <label class="tpl-label" for="tpl-iframe-maxwidth">Largeur max</label>
            <input type="text" class="tpl-input" id="tpl-iframe-maxwidth" name="maxWidth" data-field="maxWidth" value="${helpers.escapeHtml(
              data.maxWidth || "1100px"
            )}" placeholder="1100px" autocomplete="off">
          </div>
        </div>
      </div>
      ...
      <div class="tpl-form-section">
        ...
        <div class="tpl-form-grid">
          ${self.renderColorField(
            "header",
            "Barre de titre",
            data.colors?.header || "#2a2a2a"
          )}
          ${self.renderColorField(
            "background",
            "Fond",
            data.colors?.background || "#1a1a1a"
          )}
        </div>
      </div>
    `;
    ...
    overlay.innerHTML = \`
      <div class="popup-${objectName}">
        <div class="popup-${objectName}-header">
          ...
          <button class="popup-${objectName}-close" aria-label="Fermer">✕</button>
        </div>
        <div class="popup-${objectName}-loading">Chargement...</div>
        <iframe src="\${CONFIG.url}" class="popup-${objectName}-iframe" title="\${CONFIG.title}"></iframe>
      </div>
    \`;
