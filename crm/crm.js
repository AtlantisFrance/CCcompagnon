...
(function() {
    ...
    async function loadDashboard() {
        ...
        const el = document.querySelector('#section-dashboard .crm-section-content');
        el.innerHTML = `
            <!-- Stats Cards -->
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px;">
                ...
                <div class="crm-card">
                    ...
                    ${result.recent_activity.length > 0 ? `
                        <div>
                            ${result.recent_activity.slice(0, 5).map(log => `
                                <div class="crm-activity-item">
                                    <div class="crm-activity-icon ${getActivityColor(log.action)}">
                                        ${getActivityIcon(log.action)}
                                    </div>
                                    <div class="crm-activity-content">
                                        <div class="crm-activity-text">
                                            <strong>${log.first_name || 'Système'}</strong> ${getActivityText(log.action)}
                                        </div>
                                        <div class="crm-activity-time">${formatDate(log.created_at)}</div>
                                    </div>
                                </div>
                            `).join('')}
                        </div>
    ...
    async function loadUsers() {
        ...
        el.innerHTML = `
            <div class="crm-card">
                <div class="crm-card-header">
                    <h3 class="crm-card-title">👥 Liste des utilisateurs (${result.stats.total})</h3>
                    <div style="display: flex; gap: 10px;">
                        <span class="crm-badge crm-badge-success">${result.stats.active} actifs</span>
                        <span class="crm-badge crm-badge-warning">${result.stats.pending} en attente</span>
                    </div>
                </div>
                <div class="crm-table-wrapper">
                    <table class="crm-table">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div style="display: flex; gap: 6px;">
                                            <button class="crm-btn crm-btn-sm crm-btn-secondary crm-btn-icon" onclick="window.CRM.editUser(${user.id})" title="Modifier">
                                                ✏️
                                            </button>
                                            ${user.status === 'pending' ? `
    ...
    async function editUser(userId) {
        ...
        openModal('✏️ Modifier l\'utilisateur', `
    ...
    async function activateUser(userId) {
        try {
            await apiCall('/admin/users.php', 'PUT', { id: userId, status: 'active' });
    ...
    async function suspendUser(userId) {
        ...
        try {
            await apiCall('/admin/users.php', 'PUT', { id: userId, status: 'suspended' });
                            ...
                            <div class="crm-tree-header" onclick="window.CRM.toggleTreeItem(this.parentElement)">
                                ...
                                <div style="flex: 1;">
                                    ...
                                    <div style="font-size: 12px; color: var(--text-muted);">
                                        ${space.slug} • ${space.zone_count} zones • ${space.admin_count} admins
                                    </div>
                                </div>
                                <span class="crm-badge ${space.is_active ? 'crm-badge-success' : 'crm-badge-danger'}">
                                    ${space.is_active ? 'Actif' : 'Inactif'}
                                </span>
                                <div style="display: flex; gap: 6px;" onclick="event.stopPropagation()">
                                    <button class="crm-btn crm-btn-sm crm-btn-secondary crm-btn-icon" onclick="window.CRM.editSpace(${space.id})" title="Modifier">
                                        ✏️
                                    </button>
                                    <button class="crm-btn crm-btn-sm crm-btn-primary crm-btn-icon" onclick="window.CRM.createZone(${space.id})" title="Ajouter zone">
                                        ➕
                                    </button>
                                    <button class="crm-btn crm-btn-sm crm-btn-danger crm-btn-icon" onclick="window.CRM.deleteSpace(${space.id})" title="Supprimer">
                                        🗑️
                                    </button>
                                </div>
                            </div>
        ...
        if (element.classList.contains('open')) {
            ...
            try {
                ...
                if (result.zones.length > 0) {
                    zonesEl.innerHTML = result.zones.map(zone => `
                        <div class="crm-tree-child">
                            <div style="display: flex; align-items: center; gap: 10px;">
                                ...
                                <div>
                                    ...
                                    <div style="font-size: 12px; color: var(--text-muted);">
                                        ${zone.slug} • ${zone.content_count} contenus
                                    </div>
                                </div>
                            </div>
                            <div style="display: flex; gap: 6px;">
                                <button class="crm-btn crm-btn-sm crm-btn-secondary crm-btn-icon" onclick="window.CRM.editZone(${zone.id})" title="Modifier">
                                    ✏️
                                </button>
                                <button class="crm-btn crm-btn-sm crm-btn-danger crm-btn-icon" onclick="window.CRM.deleteZone(${zone.id})" title="Supprimer">
                                    🗑️
                                </button>
                            </div>
                        </div>
                    ...
                    zonesEl.innerHTML = '<div class="crm-tree-child" style="color: var(--text-muted);">Aucune zone</div>';
                }
            } catch (error) {
                zonesEl.innerHTML = '<div class="crm-tree-child" style="color: var(--danger);">Erreur de chargement</div>';
            }
        }
    }

    function createSpace() {
        openModal('➕ Nouvel espace', `
    ...
    async function editSpace(spaceId) {
        ...
        openModal('✏️ Modifier l\'espace', `
            <form id="edit-space-form">
                <input type="hidden" name="id" value="${space.id}">
                <div class="crm-form-group">
                    <label class="crm-form-label">Slug</label>
                    <input type="text" class="crm-form-input" value="${space.slug}" disabled>
                    <small style="color: var(--text-muted);">Le slug ne peut pas être modifié</small>
                </div>
                <div class="crm-form-group">
                    <label class="crm-form-label">Nom</label>
                    <input type="text" class="crm-form-input" name="name" value="${space.name}" required>
                </div>
                <div class="crm-form-group">
                    <label class="crm-form-label">Description</label>
                    <textarea class="crm-form-textarea" name="description">${space.description || ''}</textarea>
                </div>
                <div class="crm-form-group">
                    <label class="crm-form-label">URL Shapespark</label>
                    <input type="url" class="crm-form-input" name="shapespark_url" value="${space.shapespark_url || ''}">
                </div>
                <div class="crm-form-group">
                    <label class="crm-form-label">
                        <input type="checkbox" name="is_active" ${space.is_active ? 'checked' : ''}>
                        Espace actif
                    </label>
                </div>
            </form>
    ...
    async function deleteSpace(spaceId) {
        if (!confirm('Voulez-vous vraiment supprimer cet espace ? Toutes les zones et contenus associés seront supprimés.')) return;
                        ...
                        <tbody>
                            ${result.roles.map(role => `
                                <tr>
                                    ...
                                    <td>
                                        ${role.zone_name ? `<span class="crm-badge crm-badge-info">${role.zone_name}</span>` : '<em style="color:var(--text-muted)">Tout l\'espace</em>'}
                                    </td>
                                    <td>
    ...
    async function createRole() {
        ...
        openModal('➕ Assigner un rôle', `
            <form id="create-role-form">
                <div class="crm-form-group">
                    <label class="crm-form-label">Utilisateur</label>
                    <select class="crm-form-select" name="user_id" required>
                        <option value="">Sélectionner un utilisateur</option>
                        ${usersRes.users.map(u => `<option value="${u.id}">${u.first_name} ${u.last_name} (${u.email})</option>`).join('')}
                    </select>
                </div>
                <div class="crm-form-group">
                    <label class="crm-form-label">Espace</label>
                    <select class="crm-form-select" name="space_id" required onchange="window.CRM.loadZonesForRole(this.value)">
                        ...
                        ${spacesRes.spaces.map(s => `<option value="${s.id}">${s.name}</option>`).join('')}
                    </select>
                </div>
                <div class="crm-form-group">
    ...
    async function loadZonesForRole(spaceId) {
        ...
        if (!spaceId) {
            select.innerHTML = '<option value="">Sélectionner d\'abord un espace</option>';
            return;
        }
        ...
        try {
            ...
            select.innerHTML = `
                ...
                ${result.zones.map(z => `<option value="${z.id}">${z.name}</option>`).join('')}
            `;
        } catch (error) {
    ...
    async function createContent() {
        ...
        openModal('➕ Nouveau contenu', `
            <form id="create-content-form">
                <div class="crm-form-group">
                    <label class="crm-form-label">Zone</label>
                    <select class="crm-form-select" name="zone_id" required>
                        <option value="">Sélectionner une zone</option>
                        ${zonesRes.zones.map(z => `<option value="${z.id}">${z.space_name} → ${z.name}</option>`).join('')}
                    </select>
                </div>
                <div class="crm-form-group">
    ...
    async function editContent(contentId) {
        ...
        const content = contentsRes.contents.find(c => c.id == contentId);

        openModal('✏️ Modifier le contenu', `
    ...
    function init() {
        ...
        if (state.user.global_role !== 'super_admin') {
            alert('Accès refusé. Vous devez être super_admin.');
            window.location.href = '/';
            return;
        }

        // Update user info in sidebar
        const userEl = document.querySelector('.crm-user-name');
        if (userEl) userEl.textContent = `${state.user.first_name} ${state.user.last_name}`;

        const avatarEl = document.querySelector('.crm-sidebar-footer .crm-user-avatar');
        if (avatarEl) avatarEl.textContent = (state.user.first_name?.[0] || '') + (state.user.last_name?.[0] || '');
