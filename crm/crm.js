 ...
 * Version avec module PLV/Textures
 * ============================================
 */
...
(function () {
  ...
  function navigateTo(section) {
    ...
    const titles = {
      ...
      textures: { icon: "🎨", text: "Création Texture" },
  ...
  // ============================================
  // 🎨 PLV / TEXTURES SECTION
  // ============================================

  async function loadTextures() {
    // Charger les espaces et les projets PLV
            ...
            ${projects
              ...
              <div class="plv-project-card">
                <div class="plv-project-header">
                  ...
                  <span class="crm-badge ${
                    project.is_active ? "crm-badge-success" : "crm-badge-danger"
                  }">
                ...
                <div class="plv-project-body">
                  <div class="plv-project-meta">
                    ...
                    ${
                      project.zone_name
                        ? `<span class="crm-badge crm-badge-info">${project.zone_name}</span>`
                        : ""
                    }
                  </div>
                  ${
                    project.description
                      ? `<p style="font-size: 13px; color: var(--text-muted); margin: 0;">${project.description}</p>`
                      : ""
                  }
                  <div class="plv-project-stats">
                    <div class="plv-stat">
                      <div class="plv-stat-value">${
                        project.total_slots || 0
                      }</div>
                      <div class="plv-stat-label">Total slots</div>
                    </div>
                    <div class="plv-stat">
                      <div class="plv-stat-value" style="color: var(--success);">${
                        project.uploaded_slots || 0
                      }</div>
                ...
                <div class="plv-project-actions">
                  <button class="crm-btn crm-btn-sm crm-btn-primary" onclick="window.CRM.managePLVProject(${
                    project.id
                  })" style="flex: 1;">
                    🎨 Gérer les textures
                  </button>
  ...
  function updatePLVSummary() {
    ...
    const getValue = (name) =>
      parseInt(form.querySelector(`[name="${name}"]`)?.value || 0);

    const total =
      getValue("carre_opaque") +
      getValue("carre_transparent") +
      getValue("portrait_opaque") +
      getValue("portrait_transparent") +
      getValue("paysage_opaque") +
      getValue("paysage_transparent");

    const summaryEl = document.getElementById("plv-summary");
    if (summaryEl) {
      summaryEl.innerHTML = `<span>Total:</span> <strong>${total} slot${
        total > 1 ? "s" : ""
      }</strong>`;
    }
  }
  ...
  async function saveNewPLVProject() {
    ...
    const totalSlots = Object.values(data.slots_config).reduce(
      (a, b) => a + b,
      0
    );
    if (totalSlots === 0) {
      showToast("Veuillez configurer au moins un slot", "warning");
      return;
    }
  ...
  async function managePLVProject(projectId) {
    try {
      // Charger les détails du projet avec ses slots
        ...
        `
          <div style="max-height: 70vh; overflow-y: auto;">
            <div style="display: flex; gap: 12px; margin-bottom: 20px;">
              ...
              <span class="crm-badge" style="background: rgba(99, 102, 241, 0.2); color: #818cf8;">${
                project.folder_name
              }</span>
            </div>
  ...
  function renderSlotCard(slot, project) {
    const imageUrl = slot.is_uploaded
      ? `https://compagnon.atlantis-city.com/plv/${project.folder_name}/${
          slot.filename
        }?v=${Date.now()}`
      : null;
  ...
  async function handlePLVUpload(event) {
    ...
    if (!slotId) {
      showToast("Erreur: slot non identifié", "error");
      return;
    }
  ...
  async function deletePLVProject(projectId) {
    if (
      !confirm(
        "Voulez-vous vraiment supprimer ce projet PLV ?\n\nToutes les images uploadées seront supprimées."
      )
    )
      return;
